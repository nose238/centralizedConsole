...
#---------------------MAIN-------------------------------#        
        while True:
            ...
            for line in ipList:
                ...
                print(str(i) + ": " + userClient + "@" + clientIP + "  PSW: "  + pswClient + "   PORT: " + portClient )
                i += 1 
                ...
                status = commands.getstatusoutput("sshpass -p '' ssh -o StrictHostKeyChecking=no " 
                    + userClient + "@" + clientIP + " -p " + portClient) #it verifies if public key is on the Server  
                if status[0] == 0: # return 0 whit no errors. Public key on in the Client
                    print("Key found")
                    txtStatus.write("0")
                elif status[0] == 1280: #Error Client found but password does not match... somehow key is not on the Client
                    ...
                    key_status = commands.getoutput("sshpass -p \""+pswClient+"\" ssh-copy-id -o StrictHostKeyChecking=no "+userClient+"@"+clientIP+" -p "+portClient) 
